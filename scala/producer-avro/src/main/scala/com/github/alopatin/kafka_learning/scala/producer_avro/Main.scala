package com.github.alopatin.kafka_learning.scala.producer_avro
...
/**
 * Exersice: Write Kafka producer with avro serializer
 */
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
        for (_ <- 1 to count) {
          ...
          value.put("measurement", measurement)

          val record = new ProducerRecord(s"$topic-avro", key, value)

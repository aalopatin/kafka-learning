package com.github.alopatin.kafka_learning.java.producer_protobuf;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        kafkaProperties.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, KafkaProtobufSerializer.class.getName());
        ...
        var rand = new Random();
        try (
                var producer = new KafkaProducer<String, LocationMeasurement>(kafkaProperties)
        ) {
            rangeClosed(1, count).forEach(i -> {
                ...
                var value = LocationMeasurement
                        .newBuilder()
                        .setLocation(location)
                        .setMeasurement(measurement)
                        .build();

                var record = new ProducerRecord<>(topic + "-protobuf", key, value);
